...
typedef boost::multi_array < double, 2 > array_2d_real ;
...
double energy_tot(array_2d sitespin, array_2d_real J_x, array_2d_real J_y);
double nn_energy(array_2d sitespin,  array_2d_real J_x, array_2d_real J_y, unsigned int row, unsigned int col);
...
//	cin >> del_beta;
	...
	//Choose both replicas to have same spin configuration at some very high T
	for (unsigned int i = 0; i < axis1; ++i)
		for (unsigned int j = 0; j < axis2; ++j)
		{
